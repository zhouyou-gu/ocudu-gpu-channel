...
- When state moves to a new owner, ask what was DIFFERENT per old owner before assuming they were copies. Promoting a per-lane value to per-link looked like deduplication and was not: a fixed matrix stores its per-lane weights in exactly that value, so sharing one copy erased the matrix at initialisation, before any runtime path existed. The rule that fell out is worth reusing -- move the DECISION to the shared owner and leave the VALUES where they were, applying the decision to each holder in the same slot. [Lesson from M4.2, caught by an M1 gate rather than by reasoning.]
- A capability is not exercised until the LAYER THAT SHIPS IT runs it. A milestone whose gates all call one component directly proves that component and nothing above it; the integration around it can be structurally complete and never once executed. When a milestone adds a capability, run it through the top-level path (here: the broker, with synthetic peers) and lock that run into the standing sequence, or record explicitly that it is unproven there. [Lesson from M3: three milestones of MIMO work -- lane expansion, multi-row output, correlation, coherent LOS -- and no test ever put a declared multi-port topology through the broker.]
