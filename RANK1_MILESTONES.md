...
**다중 UE 확장 (2026-08-19, 리뷰 후속)**: 다중포트 gNB에 srsUE를 두 대 붙이는 게이트를 추가했다 —
`scripts/remote/ocudu-rank1-{2x1,4x1}-multi-ue-smoke.sh`. UE는 각각 1안테나를 유지하므로 사용자별로는
여전히 rank-1 MISO/SIMO이며, 새로 검증되는 것은 **다중포트 수신단에서의 중첩**이다: 두 UE의 업링크가
같은 gNB 수신 포트들에 도달하므로 각 RX 행은 두 열벡터의 합이다. 행렬 검사기를 확장해 목적지 노드로
들어오는 모든 링크를 합산해 행을 재구성하고, 링크를 하나씩 제거했을 때 일치가 깨지는지까지 요구한다
(한 사용자를 무시하는 릴레이는 통과할 수 없다).

- 2T2R + 2 UE: 두 UE 전부 attach+PDU+ping, 행 2개 max|y−ΣHx| = 4.54e-05 / 4.53e-05,
  링크 제거 시 오차 91.3~236.9 (즉 두 사용자 모두 실제로 기여)
