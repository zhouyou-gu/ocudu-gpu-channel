...
models:
  h:
    ...
    spatial_correlation:
      ...
      rx:                      # Nr x Nr, Hermitian, 대각 1
        - [ {re: 1.0, im: 0.0}, {re: 0.3, im: 0.1} ]
        - [ {re: 0.3, im: -0.1}, {re: 1.0, im: 0.0} ]
      tx:                      # Nt x Nt, 같은 조건
        - [ {re: 1.0, im: 0.0}, {re: 0.7, im: 0.0} ]
        - [ {re: 0.7, im: 0.0}, {re: 1.0, im: 0.0} ]
      # kind: full 이면 rx/tx 대신
      # matrix:  (Nt*Nr) x (Nt*Nr), lane 순서는 §2.3
```
...
측정 전에는 아무것도 적지 않는다. M3.7에서 `perf-fanin-sweep.sh`에 상관 config를 추가해 재고, 하드웨어·샘플레이트·토폴로지·모델 체인·백엔드·구간을 함께 기록한다.

예상되는 두 방향만 적어 둔다(주장이 아니라 측정 대상):
- 생성기 분리는 §1.1의 블록별 중복 계산을 없앤다 — 줄어드는 쪽.
- mixing은 slot당 `taps × grid_points × lanes²`의 복소 곱을 더한다. 4×4·23 tap·11 grid면 약 65 k 복소곱/슬롯/링크 — 늘어나는 쪽이지만 per-sample 작업(23040 샘플 × 23 tap × 8 필터탭)에 비하면 작다.

---

## 7. 열려 있는 결정

권고를 적어 두되, 뒤집는 비용이 낮은 동안 사용자가 결정할 수 있게 남긴다.

1. **`fixed_mimo` + `spatial_correlation` 동시 선언** — 거부를 권고(§2.6). 허용하면 exit 게이트가 "선언한 `R`" 대신 "그 부분행렬"을 검증하게 된다.
2. **`kind: full`의 필요성** — 밀레스톤이 `kronecker`와 `full` 둘 다 적고 있다. `full`은 `Nt·Nr` 제곱 크기 행렬을 YAML로 쓰게 하는데, 4×4면 16×16 = 256 성분이다. 실제로 쓸 계획이 없다면 M3에서는 `iid` / `kronecker`만 구현하고 `full`을 M3 이후로 미루는 편이 스키마 표면을 줄인다.
3. **LOS 기본값** — 전 lane 위상 0(rank-1)을 권고(§2.4). 대안은 `los_matrix` 미선언 시 LOS tap 자체를 거부하는 것인데, TDL-D/E 예제가 전부 걸리므로 권고하지 않는다.
