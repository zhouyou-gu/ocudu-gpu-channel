#!/usr/bin/env python3
"""Strict post-run verifier for the native OAI nrUE 1x1 attach gate (M6.2)."""
...
EXPECTED_SOURCE_COMMITS = {
    ...
    "oai": "2b69bde6aeafe892cda1531a0f0cbba2e37792cd",
...
def validate_artifacts(results_root: Path, summary_path: Path, now: float) -> dict[str, int]:
    ...
    require(require_uint(summary, "duration_seconds") == 25, "duration is not fixed to 25 seconds")
    require(require_string(summary, "ue") == "oai-nrue", "summary does not identify the OAI nrUE")
    require(
        require_string(summary, "oai_ref") == "2026.w33",
        "OAI reference is not pinned",
    )
    require(
    ...
    expected_log_dir = results_root / "logs/oai-1x1" / timestamp
    expected_report_dir = results_root / "reports/oai-1x1" / timestamp
    ...
    require(
        summary_real == results_real / "reports/oai-1x1" / timestamp / "attach-summary.json",
    ...
    require(
        evidence.get("schema") == "ocudu-native-oai-1x1-source-evidence/v1",
    ...
    require(
        ...
        and set(binary_hashes)
        == {"gnb", "nrue", "oai_zmq_module", "open5gs_5gc", "mongod", "broker"},
    ...
    expected_configs = {"gnb.yaml", "topology.yaml", "open5gs.yaml", "nrue.conf", "subscriber.csv"}
    ...
    require(require_uint(summary, "nrue_alive_at_broker_stop") == 1, "OAI nrUE exited before Broker stop")
    ...
    require(
        broker_real == results_real / "logs/oai-1x1" / timestamp / "broker.log",
    ...
    for path, label in (
        (nrue_log, "OAI nrUE log"),
        (ping_log, "UE ping log"),
        (subscriber_log, "subscriber verification log"),
        (primitive_log, "postbuild primitive probe log"),
    ):
        require(path.is_file() and not path.is_symlink(), f"{label} is invalid")
    nrue_text = nrue_log.read_text(encoding="utf-8", errors="replace")
    require("State = NR_RRC_CONNECTED" in nrue_text, "OAI nrUE log lacks RRC attach evidence")
    require(
        "Received PDU Session Establishment Accept" in nrue_text,
        "OAI nrUE log lacks PDU session evidence",
    )
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ocudu-native-oai-verifier.") as temporary:
        root = Path(temporary).resolve()
        log_dir = root / "logs/oai-1x1" / timestamp
        report_dir = root / "reports/oai-1x1" / timestamp
        ...
        broker.write_text(
            "event=radio_node_resolved id=gnb0 tx[0]=gnb0 rx[0]=gnb0 implicit=true\n"
            "event=radio_node_resolved id=ue0 tx[0]=ue0 rx[0]=ue0 implicit=true\n"
            "event=start backend=cuda duration=25s\n"
        ...
        evidence = {
            ...
            "binary_sha256": {
                name: "0" * 64
                for name in ("gnb", "nrue", "oai_zmq_module", "open5gs_5gc", "mongod", "broker")
            },
