#!/usr/bin/env python3
"""Strict post-overlay verifier for the native legacy 1x1 attach gate."""
...
def validate_artifacts(results_root: Path, summary_path: Path, now: float) -> dict[str, int]:
    ...
    require(
        evidence.get("schema") == "ocudu-native-legacy-1x1-source-evidence/v1",
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ocudu-native-verifier.") as temporary:
        ...
        log_dir = root / "logs/ocudu-interop" / timestamp
        report_dir = root / "reports/ocudu-interop" / timestamp
