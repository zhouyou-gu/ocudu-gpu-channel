...
inner="${script_dir}/run-ocudu-rank1-4x1-inner.sh"
renderer="${script_dir}/render-rank1-4x1-configs.py"
verifier="${script_dir}/verify-rank1-4x1-artifacts.py"
...
log_dir="${results_root}/logs/rank1-4x1/${timestamp}"
report_dir="${results_root}/reports/rank1-4x1/${timestamp}"
config_dir="${native_root}/configs/ocudu-rank1-4x1-native/${timestamp}"
data_dir="${native_root}/data/ocudu-rank1-4x1-native/${timestamp}"
netns_dir="${native_root}/run/ocudu-rank1-4x1-native/${timestamp}/netns"
...
if [[ "${run_status}" -ne 0 ]]; then
  printf 'event=native_rank1_4x1_attach_gate result=fail child_status=%s summary="%s"\n' \
