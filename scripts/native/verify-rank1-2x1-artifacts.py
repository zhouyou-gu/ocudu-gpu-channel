#!/usr/bin/env python3
"""Strict post-run verifier for the native rank-1 2x1/1x2 live gate (R2)."""
...
def validate_artifacts(results_root: Path, summary_path: Path, now: float) -> dict[str, int]:
    ...
    require(require_string(summary, "gate") == "rank1-2x1", "summary does not identify the rank1-2x1 gate")
    require(
    ...
    expected_log_dir = results_root / "logs/rank1-2x1" / timestamp
    expected_report_dir = results_root / "reports/rank1-2x1" / timestamp
    ...
    require(
        summary_real == results_real / "reports/rank1-2x1" / timestamp / "attach-summary.json",
    ...
    require(
        evidence.get("schema") == "ocudu-native-rank1-2x1-source-evidence/v1",
    ...
    require(
        broker_real == results_real / "logs/rank1-2x1" / timestamp / "broker.log",
    ...
    require(
        ...
        "topology did not resolve the explicit 2-port gNB node and 1-port UE node",
    )
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ocudu-native-verifier.") as temporary:
        ...
        log_dir = root / "logs/rank1-2x1" / timestamp
        report_dir = root / "reports/rank1-2x1" / timestamp
    ...
    print("event=native_rank1_artifact_verifier_self_test result=pass")
...
def main() -> int:
    ...
    print(
        "event=native_rank1_2x1_attach_gate result=pass "
