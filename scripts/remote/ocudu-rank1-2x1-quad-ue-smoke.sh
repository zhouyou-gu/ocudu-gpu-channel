#!/usr/bin/env bash
# Live rank-1 gate with FOUR srsUEs on one 2T2R cell.
#
...
OCUDU_MUE_GATE_NAME="${OCUDU_MUE_GATE_NAME:-rank1-2x1-quad-ue}" \
OCUDU_MUE_GNB_CONFIG="${OCUDU_MUE_GNB_CONFIG:-examples/ocudu/gnb_zmq_b210_fdd_2t2r_rank1_multi_ue_srsue.yaml}" \
OCUDU_MUE_TOPOLOGY="${OCUDU_MUE_TOPOLOGY:-examples/topology.ocudu-docker.rank1-2x1-quad-ue.cuda.yaml}" \
...
OCUDU_MUE_MATRIX_ARGS="${OCUDU_MUE_MATRIX_ARGS:---allow-silent-source gnb0->ue0:1 --allow-silent-source gnb0->ue1:1 --allow-silent-source gnb0->ue2:1 --allow-silent-source gnb0->ue3:1}" \
