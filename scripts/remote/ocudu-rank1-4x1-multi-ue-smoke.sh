#!/usr/bin/env bash
# Live rank-1 gate with TWO srsUEs on one 4T4R cell.
#
# This is the multi-user counterpart of ocudu-rank1-2x1-smoke.sh. The gNB keeps
# four antennas; each srsUE keeps one (nof_antennas = 1). Per user the downlink
# is a 1x4 row and the uplink a 4x1 column, so every claim stays rank-1
# MISO/SIMO -- nothing here is 2x2.
#
# What it adds over the single-UE gate is superposition at a MULTI-PORT
# receiver. Both uplinks arrive on the same four gNB receive ports, so each gNB
...
# common channels on port 0 only and precodes rank-1 PDSCH as [1, 0, 0, 0], so the
...
OCUDU_MUE_GATE_NAME="${OCUDU_MUE_GATE_NAME:-rank1-4x1-multi-ue}" \
OCUDU_MUE_GNB_CONFIG="${OCUDU_MUE_GNB_CONFIG:-examples/ocudu/gnb_zmq_b210_fdd_4t4r_rank1_srsue.yaml}" \
OCUDU_MUE_TOPOLOGY="${OCUDU_MUE_TOPOLOGY:-examples/topology.ocudu-docker.rank1-4x1-multi-ue.cuda.yaml}" \
OCUDU_MUE_GNB_TX_PORTS="${OCUDU_MUE_GNB_TX_PORTS:-2000,2002,2004,2006}" \
OCUDU_MUE_MATRIX="${OCUDU_MUE_MATRIX:-1}" \
OCUDU_MUE_MATRIX_ARGS="${OCUDU_MUE_MATRIX_ARGS:---allow-silent-source gnb0->ue0:1 --allow-silent-source gnb0->ue0:2 --allow-silent-source gnb0->ue0:3 --allow-silent-source gnb0->ue1:1 --allow-silent-source gnb0->ue1:2 --allow-silent-source gnb0->ue1:3}" \
