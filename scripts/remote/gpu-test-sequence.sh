...
#   [1/8] CUDA release build
#   [2/8] ctest (config, processing incl. the CUDA AWGN/delay checks, ring, broker)
#   [3/8] synthetic CUDA relay loop  -- clean 0 dB channel, must relay IQ both
#         ways with zero data-integrity counters
#   [4/8] synthetic CUDA relay loop  -- AWGN channel, sink power must equal
#         signal power + noise_power
#   [5/8] synthetic CUDA 3-node interference graph -- the two UE uplinks must
...
#   [6/8] synthetic CUDA 2-cell multi-gNB graph -- four nodes, eight links;
#         every RX is its serving link plus the other cell's interference
#   [8/8] synthetic CUDA 2x2 correlated MIMO relay -- a declared spatial
...
#   [7/8] synthetic CUDA TDL-A profile relay -- TR 38.901 §7.7.2 NLOS 23-tap
...
echo "== [3/8] synthetic CUDA relay loop -- clean 0 dB channel =="
...
echo "== [4/8] synthetic CUDA relay loop -- AWGN channel (noise_power 0.25) =="
...
echo "== [5/8] synthetic CUDA 3-node interference graph =="
run_graph_check "${project_root}/examples/topology.graph.cuda.yaml"

echo "== [6/8] synthetic CUDA 2-cell multi-gNB graph =="
run_multi_gnb_check "${project_root}/examples/topology.multi-gnb.cuda.yaml"

echo "== [7/8] synthetic CUDA TDL-A profile relay (TR 38.901 §7.7.2, 23-tap NLOS, Jakes 100 Hz) =="
...
echo "== [8/8] synthetic CUDA 2x2 correlated MIMO relay =="
